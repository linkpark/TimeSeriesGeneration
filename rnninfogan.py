...
#     num_dim_1 = 5
#     num_dim_2 = 5
#     cont_dim1 = np.linspace(-1.0, 0.0, num_dim_1)
#     cont_dim2 = np.linspace(-1.0, 0.0, num_dim_2)
#
#     cont_noise = []
#     for i in range(num_dim_1):
#         for j in range(num_dim_2):
#             cont_noise.append( [ cont_dim1[i], cont_dim2[j]])
#
#     cont_noise = np.array(cont_noise, dtype=np.float32)
#     unconditional_noise = np.random.normal(size=[25, sequence_length,
#                                                  unstructured_continuous_noise_dims])
...
class LSTMTFGAN:
    ...
    def _build_tf_gan_model(self):
        ...
        def infogan_generator(inputs, weight_decay=2.5e-5, is_training=True):
            unstructured_noise, cont_noise = inputs
            conditional_noise = cont_noise
        ...
        def infogan_discriminator(sequence_data, unused_conditioning, weight_decay=2.5e-5,
                                  continuous_dim=2, is_training=True):
            with framework.arg_scope(
                    [layers.fully_connected],
        ...
        with tf.variable_scope('Generator', reuse=True):
            ...
            noise = [self.unconditional_noise, self.cont_noise]
    ...
    def inference(self, sess, unconditional_noise, cont_noise):
    ...
    def train(self, save_path, test_noise_generator, visualization=True):
        ...
        unconditional_noise, cont_noise = test_noise_generator()
        ...
        with tf.Session() as sess:
            ...
            for i in range(self.args.epoch):
                ...
                if i % self.args.interval == 0:
                    display_seq_np = self.inference(sess, unconditional_noise, cont_noise)
